...
def get_months_in_year(year, start, end=None):
    months = []
    if not end:
        if start < 10:
            start = f'0{str(start)}' 
...
def download_all_data(year, start_month,end_month):
    year, months = get_months_in_year(year,start_month,end_month)
...
def download_data(symbol ,year, start_month,end_month=None):
    year, months = get_months_in_year(year,start_month,end_month)
    url_pattern = local.task_urls["symbol_data_link_pattern"]
    for i in months:
        url = f'{url_pattern%symbol}/{symbol}-1m-{year}-{i}.zip'
...
async def main():
    ...
    if await waiting_url_done(local.task_urls["symbol_list_url"]):
        await get_symbols()
    # download_all_data(2023,1,11)
        download_data("BTCUSDT",2023,11)
        download_data("ETCUSDT",2023,11)
    input("end?")
